...
for (let i = 0; i < countStb; i ++){
	stb.push(parameters.parameters[i].name + " double precision NOT NULL");
...
new Promise(function(response, reject){
	client.connect(function(err) {
	  ...
	  client.query("DROP TABLE test", function(err, res){
			if (err) throw err;
...
.then(
	() => {
		for (let i = 0; i < td.length; i ++){
			...
			client.query("INSERT INTO test VALUES(" + td[i].id + "," + values.join(",") + "," + td[i].r + "," + td[i].g + "," + td[i].b + ")", function(err, res){
				if (err) throw err;
